...
where computation_date >= '2018-10-30' and internal = 'external'
group by 1
...
select 
	computation_date, 
	count(distinct case when computation_date = join_date then account_id end) as new_account,
	sum(count_new_authors) as new_authors
from quicksight.events_daily
where  computation_date >= '2018-10-30' and internal = 'external'
...
select 
	...
	sum(net_revenue) as net_revenue, --matched
	sum(count_provisioned_author),
	sum(spice_gb) as spice_gb --matched
from quicksight.revenue_daily
where computation_date >= '2018-11-13' and internal = 'external'
and payer_account_id = '153619863257'
group by 1,2
...
select 
	account_id, company_name, edition, 
...
select 
	...
	--user is sum of author and reader
	sum(count_provisioned_author) + sum(provisioned_readers) as user,
	sum(net_revenue) as net_revenue, --matched
	sum(spice_gb) as spice_gb --matched
from quicksight.revenue_daily
where computation_date >= '2018-11-13' and internal = 'external'
...
select account_id, company_name, sum(count_analysis_created)
from quicksight.events_daily
where computation_date >= '2018-11-13' and internal = 'external'
group by 1, 2
